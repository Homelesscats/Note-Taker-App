...
// add a new note to the db.json using the POST method
notesRouter.post("/notes", (req, res) => {
  ...
  // on top of user input, a unique ID is generated for the new Note using uuidv4 package.
  if (title && text) {
    const newNote = {
      title,
      text,
    ...
    fs.readFile(
      ...
      (error, storedNotes) => {
        ...
        if (error) {
          ...
          fs.writeFile(
            path.join(__dirname, "../db/db.json"),
            JSON.stringify(notesStorage, null),
            (error) =>
              error
                ? console.error(error)
                : console.info(
  ...
  fs.readFile(
    ...
    (error, storedNotes) => {
      if (error) {
        ...
        const notesStorage = JSON.parse(storedNotes);
        let findNote = notesStorage.filter(
          (deletedNote) => deletedNote.id == req.params.id
        );
        ...
        fs.writeFile(
          ...
          (error) =>
            error
              ? console.error(error)
              : console.info(
                  `Successfully deleted note with ID ${deletedNote.id}!\nYour notes will be updated shortly!`
                )
        );
